<?php 
  ...
  if(isset($_POST['booking_analytics']))
  {
    ...
    $result = mysqli_fetch_assoc(mysqli_query($con,"SELECT 
      ...
      COUNT(CASE WHEN booking_status!='Đã Đặt'AND booking_status!='payment failed' THEN 1 END) AS `total_bookings`,
      SUM(CASE WHEN booking_status!='Đã Đặt' AND booking_status!='payment failed' THEN `trans_amt` END) AS `total_amt`,

      COUNT(CASE WHEN booking_status='Đã Thanh Toán' AND arrival=1 THEN 1 END) AS `active_bookings`,
      SUM(CASE WHEN booking_status='Đã Thanh Toán' AND arrival=1 THEN `trans_amt` END) AS `active_amt`,

      COUNT(CASE WHEN booking_status='Đã Huỷ' AND refund=1 THEN 1 END) AS `cancelled_bookings`,
      SUM(CASE WHEN booking_status='Đã Huỷ' AND refund=1 THEN `trans_amt` END) AS `cancelled_amt`
