<?php 
  ...
  if(isset($_POST['get_bookings']))
  {
    ...
    $query = "SELECT bo.*, bd.* FROM `booking_order` bo
      ...
      WHERE ((bo.payment_status='Đã Thanh Toán') 
      OR (bo.payment_status='Đã Thanh Toán 50%')
      OR (bo.payment_status='Đã Huỷ')
    ...
    {   
      ...
      $price = number_format($data['price'], 0, ',', '.'); // Định dạng giá phòng
      $trans_amt = number_format($data['trans_amt'], 0, ',', '.'); // Định dạng giá phòng
      if($data['booking_status']=='Đã Thanh Toán'){
        $status_bg = 'bg-success';
      }
      else if($data['booking_status']=='Đã Thanh Toán 50%'){
      ...
      $table_data .="
        <tr>
          ...
          <td>
            ...
            <br>
            <b>Thanh Toán:</b> $trans_amt vnđ
          </td>
          <td>
            <span class='badge $status_bg'>$data[booking_status]</span>
          </td>
      ";
      if($data['booking_status']=='Đã Thanh Toán' || $data['booking_status']=='Đã Thanh Toán 50%')
      {
        $table_data.="<td>
