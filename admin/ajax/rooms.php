<?php 
  ...
  if(isset($_POST['add_image_url']))
  {
    ...
    // Download image from URL with headers (support both HTTP and HTTPS)
    ...
    // Use path relative to admin/ajax/ directory (go up 2 levels to reach root)
    $img_path = dirname(__DIR__, 2) . '/images/rooms/' . $img_name;

    // Create directory if not exists
    $dir = dirname(__DIR__, 2) . '/images/rooms/';
    if(!is_dir($dir)){
      mkdir($dir, 0777, true);
    }
    ...
    if($result !== false && $result > 0 && file_exists($img_path)){
      $q = "INSERT INTO `room_images`(`room_id`, `image`) VALUES (?,?)";
      $values = [$frm_data['room_id'],$img_name];
      $res = insert($q,$values,'is');
      echo $res;
    }
    else{
