    ...
    <div class="row">
      ...
      <?php 
        ...
        if(isset($_GET['resultCode']) && isset($_GET['orderId'])){
          ...
          if($resultCode == 0){
              update("UPDATE booking_order SET booking_status=? WHERE order_id=?",
                  ['Đã Thanh Toán', $orderId], 'ss');
          } else {
              update("UPDATE booking_order SET booking_status=? WHERE order_id=?",
                  ['payment failed', $orderId], 'ss');
          }
      }
        ...
        if($booking_fetch['booking_status']=="Đã Thanh Toán")
        {
          echo<<<data
